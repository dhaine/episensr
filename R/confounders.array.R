...
#'
#' @param crude_risk Crude (apparent or observed) relative risk between the exposure
...
#' \item{adj_measures}{Output results, with bias as a percentage: (crude_RR - risk_adj)/risk_adj * 100.}
#'
#' @family confounding
#'
...
confounders.array <- function(crude_risk,
                              ...
                              bias_parms = NULL) {
    if (length(type) > 1)
        stop(cli::format_error(c("i" = "Choose between binary, continuous, or RD implementation.")))
...
the following components: (1) Association between the confounder and the outcome, (2) Prevalence of the confounder among the exposed, and (3) Prevalence of the confounder among the unexposed.")))
...
outcome should be >= 0.")))
    if (crude_risk < 0 & type != "RD")
        stop(cli::format_error(c("x" = "Crude risk should be greater than 0.")))
    if ((crude_risk > 1 | crude_risk < -1) & type == "RD")
        stop(cli::format_error(c("x" = "Crude risk should be between -1 and 1.")))
    ...
    if (type == "binary") {
        ...
        bias_perc <- (crude_risk - risk_adj) / risk_adj * 100
    }

    if (type == "continuous") {
        risk_adj <- exp(log(crude_risk) - (log(bias_parms[1]) * (bias_parms[2] - bias_parms[3])))
        bias_perc <- (crude_risk - risk_adj) / risk_adj * 100
    }

    if (type == "RD") {
        risk_adj <- crude_risk - (bias_parms[1] * (bias_parms[2] - bias_parms[3]))
        bias_perc <- (crude_risk - risk_adj) / risk_adj * 100
    }
