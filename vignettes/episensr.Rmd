...
unexposed.
The output shows the observed 2X2 table, the same table corrected for
the selection proportions, the observed odds ratio (and relative risk) followed
by the corrected ones, and the input parameters, including the selection bias
odds ratio.

## Uncontrolled Confounders

We will use date from a cross-sectional study by
...
We choose a risk ratio implementation, provide a vector defining the prevalence
of the confounder, religion, among the exposed and the unexposed, and give the
risk ratio associating the confounder with the disease.
...
The output gives the crude 2X2 table, the 2X2 tables by levels of the
confounder, the crude relative risk and confounder specific measures of
...
Finally, the bias parameters are shown: the association between the confounder
and the outcome among those who were not exposed, the prevalence of the
confounder among the exposed, and the prevalence of the confounder among the
unexposed.
...
([Fink & Lash](http://www.ncbi.nlm.nih.gov/pubmed/12946045), where we assume
...
We saved the `probsens` analysis in a new variable `smoke.nd`.
We can see the element of the object `probsens` with the function `str()`:
...
adjusted measures.
We can plot the Se prior distribution (not forgetting to discard the draws that
...
```{r, boot_fig, fig.cap = "Bootstrap confidence interval.", warning=F}
